...
|------|------|
| 当前阶段 | 阶段三：专业与上线 |
| 当前章节 | 第 10 章：Git 进阶与远程协作（进行中，已完成 README + 建表脚本） |
| 下一步 | 第 10 章继续：分支 / Pull Request |
| 已完成 | 环境搭建 + Git + 第 3~9 章 + 第 10 章第一步（README、建表脚本） |

---

## 二、记录（按时间倒序）

### 2026-09-11 · 第 10 章开始：README + 建表脚本

**做了什么：**
- 盘点现状：远程仓库✅、.gitignore✅、敏感信息✅（db_demo.py 已 gitignore）、README 缺失、无建表脚本。
- 写 schema.sql（users + tasks 建表语句），补全"别人能初始化数据库"的缺失。
- 写 README.md（简介/功能/技术栈/目录结构/快速开始/环境变量/接口）。

**学到了什么：**
- 一份合格 README 的结构：简介 → 功能 → 技术栈 → 目录结构 → 快速开始 → 环境变量。
- 建表脚本（schema.sql）：把建库建表语句存成文件，一条命令初始化，别人才能跑起来。
- 让项目"可克隆、可运行"：光有代码不够，还要文档 + 建表脚本 + 环境变量说明。
